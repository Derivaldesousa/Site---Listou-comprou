...
<header>
    ...
    if ($result->num_rows > 0) {
        ...
        <img src="<?php echo $row['IMAGEM']; ?>" style="width: 100%; height: 100%;" alt="Imagem do produto">
        ...
        <div class="product-details">
            ...
            <div class="product-buttons">
                <form action="funcoes.php" method="POST">
                    <input type="hidden" value="adicionar_ao_carrinho" name="action">
                    <input type="hidden" value="<?php echo $_SESSION['id_usuario']?>" name="id_usuario">
                    <input type="hidden" value="<?php echo $row['NOME']?>" name="nome_produto">
                    <button id="btn_adicionar">ADICIONAR</button>
                </form>
                <form action="funcoes.php" method="POST">
                    <input type="hidden" value="retirar_do_carrinho" name="action">
                    <input type="hidden" value="<?php echo $row['NOME']?>" name="nome_produto">
                    <button id="btn_retirar">RETIRAR</button>
                </form>
            </div>
        </div>
    </div>
...
function filtrarProdutos(categoria, id) {
  ...
  $.each(products, function(index, product) {
    ...
    var inputNomeAdd = $('<input>').attr('type', 'hidden').val(product.name).attr('name', 'nome_produto');
    var inputIdUsuarioAdd = $('<input>').attr('type', 'hidden').val(product.id).attr('name', 'id_usuario');
    var btnAdicionar = $('<button>').attr('id', 'btn_adicionar').text('ADICIONAR');
    
    var formRetirar = $('<form>').attr('method', 'POST').attr('action', 'funcoes.php');
    var inputNomeRetirar = $('<input>').attr('type', 'hidden').val(product.name).attr('name', 'nome_produto');
    var btnRetirar = $('<button>').attr('id', 'btn_retirar').text('RETIRAR');
    
    productButtons.append(formAdd);
    formAdd.append(inputNomeAdd);
    formAdd.append(inputIdUsuarioAdd);
    formAdd.append($('<input>').attr('type', 'hidden').val('adicionar_ao_carrinho').attr('name', 'action'));
    formAdd.append(btnAdicionar);
    
    productButtons.append(formRetirar);
    formRetirar.append(inputNomeRetirar);
    formRetirar.append($('<input>').attr('type', 'hidden').val('retirar_do_carrinho').attr('name', 'action'));
...
window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (message === 'error') {
            ...
            popup.innerHTML = 'Erro a adicionar produto no seu carrinho';
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (message === 'success') {
            ...
            popup.innerHTML = 'Produto adicionado ao seu carrinho';
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (message === 'existe') {
            // Cria a div do popup
            const popup = document.createElement('div');
            popup.className = 'popup_existe';
            popup.innerHTML = 'O produto já existe no seu carrinho';
            // Adiciona a div ao corpo do documento
            document.body.appendChild(popup);
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (message === 'error_remover') {
            // Cria a div do popup
            const popup = document.createElement('div');
            popup.className = 'popup_existe';
            popup.innerHTML = 'Erro ao remover produto';
            // Adiciona a div ao corpo do documento
            document.body.appendChild(popup);
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
        ...
        if (message === 'success_remover') {
            // Cria a div do popup
            const popup = document.createElement('div');
            popup.className = 'popup_success';
            popup.innerHTML = 'O produto removido do seu carrinho';
            // Adiciona a div ao corpo do documento
            document.body.appendChild(popup);
